...
class PCAPFlowExtractor:
    """PCAP文件流量提取工具 - 支持SSH/HTTP/HTTPS流量的高性能版本"""
    ...
    def __init__(self):
        ...
        self.protocol_ports = {
            'ssh': [22],
            'http': [80, 8080, 8000, 8008, 3000, 5000, 9000],  # 常见HTTP端口
            'https': [443, 8443, 9443]  # 常见HTTPS端口
        }

    def process_pcap(self, pcap_file, max_flows=128, min_packets=10, max_scan_packets=None,
                     payload_only=True, min_interval_ms=0.1, max_interval_ms=1000, protocol='ssh'):
        """扫描PCAP文件并识别满足条件的流
        ...
        参数:
            ...
            protocol: 要提取的协议类型，支持'ssh', 'http', 'https'，默认为'ssh'
        """
        ...
        if protocol not in self.protocol_ports:
            raise ValueError(f"不支持的协议: {protocol}. 支持的协议: {list(self.protocol_ports.keys())}")
        
        target_ports = self.protocol_ports[protocol]
        protocol_name = protocol.upper()

        print(f"处理PCAP文件: {pcap_file}...")
        print(f"目标: 提取 {max_flows} 个单向流，每个流至少 {min_packets} 个有效数据包")
        print(f"过滤条件: {protocol_name}流量, 端口={target_ports}, 有效负载={payload_only}, 时间间隔={min_interval_ms}ms~{max_interval_ms}ms")
        ...
        try:
            ...
            with PcapReader(pcap_file) as pcap_reader:
                ...
                for packet in pcap_reader:
                    ...
                    # 处理数据包
                    self._process_packet_optimized(packet, payload_only, target_ports, protocol_name)
    ...
    def _process_packet_optimized(self, packet, payload_only=True, target_ports=None, protocol_name="SSH"):
        ...
        try:
            if IP in packet and TCP in packet:
                ...
                if target_ports is None:
                    target_ports = [22]  # 默认SSH端口
                
                # 只处理目标协议流量 (源端口或目标端口在目标端口列表中)
    ...
    def print_flows(self, min_packets=10, min_interval_ms=0.1, max_interval_ms=1000,
        ...
        print(f"{'=' * 90}")
        print(f"{'#':<4} {'协议':<8} {'流标识':<50} {'原始/有效间隔':<20} {'持续时间(秒)':<15}")
        print(f"{'-' * 90}")
        ...
        for i, (flow_key, flow_data) in enumerate(sorted_flows[:top_n]):
            ...
            print(f"{i:<4} {protocol:<8} {flow_key:<50} {counts:<20} {duration:.2f}")
    ...
    def extract_time_intervals(self, flow_key, flow_data, min_interval_ms=0.1, max_interval_ms=1000):
        ...
        if intervals:
            ...
            print(f"流 {flow_key} ({protocol}) 的间隔统计 (有效间隔={len(intervals)}):")
            print(f"  最小: {min_val:.3f}ms, 最大: {max_val:.3f}ms, 平均: {avg_val:.3f}ms")
        else:
            print(f"流 {flow_key} ({protocol}) 没有有效的时间间隔")
    ...
    def extract_flows(self, output_dir, min_packets=10,
        ...
        print(
            f"流过滤条件: {'有效负载' if payload_only else '所有数据包'}, 时间间隔 {min_interval_ms}~{max_interval_ms}ms，"
        ...
        for i, flow_key in enumerate(flow_keys):
            try:
                ...
                protocol = flow_data.get('protocol', 'UNKNOWN')
                base_filename = f"flow_{i:03d}_{protocol.lower()}_{safe_name}"
                ...
                print(f"流 {i:03d}: {protocol} - {flow_key} - 包:{packets_count}, 有效:{valid_packets} -> {', '.join(outputs)}")
...
def main():
    """主函数 - 支持多协议的流提取优化版本"""
    parser = argparse.ArgumentParser(description="多协议PCAP流量提取与时间间隔分析工具")
    ...
    parser.add_argument("--protocol", "-t", choices=['ssh', 'http', 'https'], 
                        default='ssh', help="要提取的协议类型，支持ssh/http/https，默认为ssh")

    args = parser.parse_args()

    packet_type = "所有" if args.all_packets else "有效负载"
    protocol_name = args.protocol.upper()

    print(f"\n=== 多协议PCAP流量分析工具 ===")
    print(f"过滤条件: 单向流, {packet_type}数据包, {protocol_name}流量, 时间间隔 {args.min_interval}~{args.max_interval}ms")
